...
- **Iteration round 2 additions** (2026-08-08, page `02 Components` unless noted): `Button` gained non-variant property `Show label#275:0` (BOOLEAN, default `true`) bound to `Label.visible` on all 16 labelled variants — the 4 `Loading` variants have no `Label` layer and are correctly unbound. New sets: `ProgressBar` = `276:51` (`Step=One|Two|Three|Four`, 25/50/75/100%; **not** auto-layout — the `Fill` child carries `SCALE`/`STRETCH` constraints so instances stay proportional on resize); `InviteCodeField` = `278:103` (`State=Empty|Partial|Complete|Error`; 32×48 cells so eight plus a dash fit the 352px auth panel, radius bound to `radius-m` = 6 matched to `PinField` by measurement). New loose components: `IconChevronDown` = `289:43`, `IconChevronUp` = `289:45`. New frame on `07 Auth & Settings`: `Settings — Server (custom)` = `281:235` (holds the URL field and connection test displaced when the Server tab moved to a Cloud-first default). **A refinement to the recorded `setExplicitVariableModeForCollection` opacity hazard:** re-asserting the mode a page is *already* in does **not** re-trigger the fractional-opacity reset — verified by repairing, then re-asserting Light, then re-reading all 819 instances across the six touched pages and finding zero drift. The hazard is specific to an actual mode *change*.
- **Iteration round 3 additions** (2026-08-10): `Button` gained `Show icon#313:0` (BOOLEAN, default `false`) bound to `Icon.visible` on the same 16 labelled variants; the 4 `Loading` variants have no `Icon` layer and are correctly unbound. **This fixed a latent defect, not just an omission** — the set already exposed an `Icon#139:0` `INSTANCE_SWAP` property, but the `Icon` child was `visible: false` in every variant with no boolean bound to it, so the swap could never render anything and the property was decorative. Because the new default is `false`, all 70 pre-existing `Button` instances (39 on `05 Review`, 21 on `07 Auth & Settings`, 10 on `06 Onboarding`, 0 on `03 Shell & Navigation`) were verified unchanged by reading the property back on each. **A gotcha worth recording: an icon in a `Button` does not inherit the label's colour.** Figma has no `currentColor`, so a swapped-in icon keeps whatever its own source component binds — `IconPlus` is bound to `ink`, which rendered a black plus beside the Ghost variant's `indigo` label. Every consumer must rebind the nested vector's stroke per instance (`setBoundVariableForPaint`, remembering it returns a *new* paint to reassign); the 27 `Add` buttons on `04 Section editor` each do this to `indigo` `4:9`. Recolouring at the component level was rejected as fragile: the override is keyed to the currently-swapped `Icon` main component, so it would not survive a swap.
- **Iteration round 4 additions** (2026-08-10): `FillSummary` (`58:30`) moved off Geist Mono to **Geist Regular** with letter-spacing 0, and `State=Empty` reworded `empty` → `Nothing yet`; variant widths changed (43→70, 51→35, 51→35) so the set frame was refitted 250×18 → 239×60. `Badge` (`58:23`) gained **`Tone=Warning`** = `328:63` (`warning` `4:16` at 0.12 opacity, label `4:16`), used for `dislike`/`avoid`, whose original design is amber not red. New loose components: `IconThumbsUp` = `328:59`, `IconThumbsDown` = `328:61` (16×16, stroke 1.6666667461395264, ROUND cap/join, two subpaths — cuff plus mitten; the down glyph is the up glyph mirrored as `y' = 16 - y`). New frames on `04 Section editor`: `Desktop — Add entry` = `320:787` (x=10390, modal `320:950`), `Desktop — Add entry (duplicate)` = `322:888` (x=12030), `Desktop — Filters active` = `324:989` (x=13670), `Desktop — Sort` = `327:1076` (x=15310), `Mobile — Add entry` = `330:1149` (x=16950, 390×844, `clipsContent`). **Two API hazards recorded, both cost a rebuild:** (1) **`setBoundVariableForPaint` silently discards a fractional `opacity` set on the paint object passed into it** — the returned paint comes back at `opacity: 1`. Applying opacity must be a *second* write (`const f = node.fills.map(clone); f[0].opacity = 0.12; node.fills = f`), verified by reading back. This produced a fully opaque black scrim and a solid amber badge before it was caught. (2) The `Badge` fractional-opacity reset is re-triggered by **`node.clone()` on any ancestor** and by **every `setProperties` variant switch** — so the repair pass must run *after* all cloning and all variant switches, not before. Repairing first and switching variants second leaves fresh drift, which happened once on `Desktop — Filters active`. Dialog geometry, cloned from `Desktop — Promote dialog` (`129:348`): `Scrim` RECTANGLE at full frame size bound to `scrim` `140:2` at 0.4 paint opacity, `Modal` VERTICAL auto-layout 400 wide (352 content) padded and gapped at `space-24`, radius `radius-xl`, `DROP_SHADOW` radius 32 offset (0,12) `ink` at 0.14.
- **Iteration round 5 additions** (2026-08-10, colour foundations reconciliation against the app spec — see `design/app-contrast-audit.md`): two new colour variables and three corrected ones from the reconciliation's Task 1: `link` = `VariableID:337:2821` (Light `rgb(61,93,219)`, Dark `rgb(97,127,250)`, scopes `TEXT_FILL`/`STROKE_COLOR`), `on-primary` = `VariableID:337:2822` (`rgb(255,255,255)` both modes, deliberately mode-invariant, same scopes); `muted-fg` (`VariableID:4:7`) Light corrected `rgb(120,114,109)` → `rgb(113,106,102)`; `indigo` (`VariableID:4:9`) Dark corrected `rgb(92,123,250)` → `rgb(67,103,249)`; `indigo-tint` (`VariableID:4:10`) Dark darkened `rgb(40,45,62)` → `rgb(28,26,45)` so the rail's active row clears AA. Seven new variables from Task 2, replacing the badge tones' 12%-opacity self-tint wash with designed tint/ink pairs at opacity 1: `success-tint` = `VariableID:339:2`, `warning-tint` = `VariableID:339:3`, `destructive-tint` = `VariableID:339:4` (scopes `FRAME_FILL`/`SHAPE_FILL`); `indigo-ink` = `VariableID:339:5`, `success-ink` = `VariableID:339:6`, `destructive-ink` = `VariableID:339:7`, `warning-ink` = `VariableID:339:8` (scopes `TEXT_FILL`/`STROKE_COLOR`). Task 3 rebound 26 TEXT nodes from `indigo` to `link` (22 on component sets, 4 stragglers found on `08 Motion`) and the 4 Primary-button labels from `card` to `on-primary`; **the labels' real node IDs are `55:3`/`56:3`/`56:5`/`56:7`** (`Primary` × `Default`/`Hover`/`Pressed`/`Disabled`) — an earlier draft of this plan named `55:2`/`56:2`/`56:4`/`56:6`, which are the four variant *components* themselves, not the `Label` text child inside each; anyone cross-referencing those older IDs against a live file should resolve to the label, not the variant. Final census: zero TEXT nodes bound to `indigo` across all nine pages; 45 non-text `indigo` bindings remain on `02 Components` by design (focus rings, switch tracks, checkbox grounds, Primary button grounds, spinner strokes) — `indigo` is fills-only now. **Two hazards this round confirmed, both worth carrying into any future token-value change:** (1) a disabled variant's dimming can be node-level `opacity` rather than a paint property (confirmed on all four non-`Plain`/`Underline` `Button` `State=Disabled` variants, still `0.5` after the badge rebind), so any script that "normalises" opacity by writing paint-level `opacity: 1` must explicitly skip disabled-state nodes or it will silently un-dim them — this round's sweep was scoped to Badge-set instances only and never touched a disabled node, but the next general opacity sweep needs its own disabled-state exclusion, verified by reading back rather than assumed clean. (2) Moving every badge ground from a 12%-opacity self-tint wash to a dedicated tint variable at opacity 1 **retires the fractional-opacity drift bug for good** — the whole "reconstructs the paint and resets opacity to 1" failure family logged repeatedly against `Badge` in Tasks 6, 9, 10 and 12 (`combineAsVariants`, `setExplicitVariableModeForCollection`, `.clone()`, `setProperties`, even a bare `createInstance()`) was only ever a problem because the correct value was fractional; now that the correct value **is** 1, none of those operations can drift it away from correct again, and the badge-specific opacity-reapply workaround these tasks all carried is no longer needed going forward.
- **Iteration round 6 additions** (2026-08-10, prototype self-consistency — every title and rail state matched to the content its frame actually renders): `Desktop — Sort` (`327:1076`) had been built by duplicating a Preferences screen and editing the wrong node. Its section title `327:1117` read `Preferences`, now `Learning log` — the subtitle `327:1118` ("What you learned, newest first") was already correct Learning log copy and was left alone. The *rail* item in slot 2, layer name `RailItem — Preferences`, carried a text override reading `Learning log` (`I327:1097;75:10`, now `Preferences`), so the rail listed "Learning log" twice and never listed Preferences at all. **The layer name and the rendered label disagreed, which is why a full review round missed it:** any audit that reads layer names instead of `characters` is structurally blind to an override, and this one surfaced only in a screenshot. Re-auditing every rail on pages `03`–`06` by comparing rendered `characters` against layer names, and each frame's header against its active rail item's label, found nothing else of the kind. The scroll-spy `Current` marker was separately stuck on `RailSubItem — Communication` across six frames whose content is not a Communication group; it now sits on the group each frame renders — `Code Style` on `Desktop — Preferences` (`109:2`, which renders the section from the top), `Likes & Dislikes` on `115:253`, `307:636`, `320:787`, `322:888` and `324:989` (12 instances re-propertied: `109:24`/`109:25`, `115:276`/`115:278`, `307:659`/`307:661`, `320:810`/`320:812`, `322:911`/`322:913`, `324:1012`/`324:1014`). `Desktop — Chip input` (`115:651`) was deliberately left on `Communication` and is correct — its group is `Response Format`, which genuinely belongs to Communication; it is the control case proving the check discriminates rather than rewriting every frame. Finally, `Mobile — Add entry` (`330:1149`) showed a sheet headed "Add to Likes & Dislikes" — whose `Stance` field exists for no other group — sitting over the Code Style cards. Here the sheet was right and the page behind it was wrong, so the content is now collapsed to the group the sheet names by **hiding, not deleting**, `330:1168`, `330:1188`, `330:1219`, `330:1220` and `330:1227`; this mirrors `Desktop — Add entry` (`320:787`), whose `Group 1` holds the `Likes & Dislikes` card alone with no eyebrow band, and the `Content` frame's VERTICAL auto-layout collapsed 2303 → 369 on its own. **Carry forward:** a variant swap can drop a text override, so this round preloaded every rail label's font before calling `setProperties` and re-read each label afterwards to repair drift — none occurred, but the check is the only thing that would have caught it.
...
**Task 13 follow-up (cross-task fix: `Tabs` component repaired after Task 7 sign-off):** the ruling (f) finding above was upgraded from "documented" to "fixed at the source." `clipsContent` was set to `false` on all 29 `Tab` frames inside the shared `Tabs` component set (`78:43`, page `02 Components`) — every `Tab` frame across all 9 variants: `Count=Two` (`78:16`, `78:22`, `78:29`, `78:35`), `Count=Three` (`173:28`, `173:32`, `173:36`, `173:41`, `173:45`, `173:49`, `173:54`, `173:58`, `173:62`), `Count=Four` (`194:28`, `194:32`, `194:36`, `194:40`, `194:45`, `194:49`, `194:53`, `194:57`, `194:62`, `194:66`, `194:70`, `194:74`, `194:79`, `194:83`, `194:87`, `194:91`) — all read back `clipsContent:false` after the edit, and the component set's own bounding box was unchanged (`1334×176`), so the stale-bounding-box quirk did not recur this time (no variant append, only a boolean toggle). The ruling (f) test frame (`206:62`, page `08 Motion`) contains a detached, non-instance copy (`210:79`, `FRAME`, named `Tabs`, `cornerRadius:0`) taken from the now-fixed `Count=Two, Active=First` component, and its `Indicator` (`210:84`) was keyframed `TRANSLATION_X` 0→92 / `WIDTH` 68→118 over 200ms `cubic-bezier(.4,0,.2,1)`; `export_video` + frame extraction confirmed the indicator genuinely slides from under the first tab to under the second and settles there. **Correction (final fix round):** this frame is a detached copy, not a live instance — `210:79` was missed by the Step 4 detach sweep and is now recorded in the spec's divergence entry as the file's second found-but-unenumerated detach. Being detached, its `clipsContentInherited:false` proves nothing about the component itself, only that this one static copy has `clipsContent` off; the earlier note that it "proves the fix came from the component" was wrong and has been struck. The component repair is evidenced separately and properly: all 29 `Tab` frames read back `clipsContent:false` across all nine variants directly on `78:43` (above), plus a codegen check that no `Tab`-level div carries `overflow-clip`. The frame's caption (`206:63`) was rewritten again in the final fix round to say this plainly. Regression-checked against one Task 10 frame (`Desktop — Inbox`, `125:2`) and one Task 12 frame (`Settings — Account`, `184:95`) by screenshot: both render unchanged, tabs and indicators correctly positioned, nothing clipped. Also bound the two previously-hardcoded `32px` gaps (`201:56`, `205:62`) to `space-32` (`VariableID:7:12`), and a page-wide `itemSpacing` sweep found and bound five more author-created gaps that had a legal value but no binding (`201:108`/`201:110`/`201:112` → `space-8`; `204:124` → `space-16`; `206:62` → `space-12`); the remaining 33 unbound hits are all inherited `itemSpacing` from cloned/instanced Task 7/8/10 content (`RailSubItem`'s internal `0`, the cloned Inbox row list's internal `0`/`8`s, and the `Tabs` `Badge` instances' `4`), unchanged from the original audit's inherited-debt list. Variable mode confirmed `4:0` (Light) on all four touched pages (`02 Components`, `05 Review`, `07 Auth & Settings`, `08 Motion`) — no mode switch performed.
...
- [x] **The delegate offer now appears before the field groups.** Originally failed: the redesign's Task 11R brief placed the offer on `— How you like answers` (step 2 of 4), after that step's own fields, and it was absent from `Welcome`/`About you` entirely. Coordinator confirmed the placement instruction, not the build, was at fault. Fixed by moving the card (not a copy) into `Onboarding — Welcome`'s column, below the heading/supporting line and above `Get started`, with its copy generalised for a pre-field context. Re-run confirms the offer's copy now appears exactly once in the current flow (`Welcome`) plus once in the retained pre-redesign `Desktop — Delegate offer` view, and zero times on `How you like answers`. See the spec's Prototype divergences section.
