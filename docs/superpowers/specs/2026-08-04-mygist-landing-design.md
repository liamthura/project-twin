...
| Display face | Stack Sans Notch (Koto, variable 200–700). **Not on Google Fonts** — see "Still open: the display face". |
...
**The decisions table above claimed this face was on Google Fonts. It is not.**
`fonts.googleapis.com/css2` returns 400 for `Stack Sans Notch`, `Stack Sans` and
`Koto`, with the same request shape that returns 200 for `Geist`. The table has
been corrected. Whoever wrote that line had a route to the file that this
repository does not, and following it costs an afternoon.

Nor is the family installed on the machine the design was built on — neither
`~/Library/Fonts` nor `/Library/Fonts` has it — so Figma was reading it from
somewhere else, and where is not recorded.

That leaves three ways out, and all three are the owner's call:

1. **Buy the webfont licence** and drop `stack-sans-notch.woff2` into
   `frontend/public/fonts/`. Nothing else changes.
2. **Substitute an openly-licensed display face.** This re-opens the 40px floor,
   which is a property of *this* face — the specimen frame on 01 Foundations was
   the gate that set it, and a different face wants that gate run again.
3. **Drop the display face**, and set headings in Geist at a heavier weight. The
   page already renders this way, so this is the status quo made deliberate.

What must not happen is a font file copied out of a system fonts folder. That is
the one option that looks like it works and is not licensed.
