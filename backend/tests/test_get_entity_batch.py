...
def test_get_entity_batch_rejects_non_dict_resolver_output(as_user, monkeypatch):
    # F3: _resolve_entity is expected to return either a JSON object with an
    # "entity" key, or a plain (non-JSON, or JSON-but-wrong-shape) error
    # string. A resolver that returns *some other* valid JSON value (here, a
    # JSON-encoded string) must still be treated as an error element, not
    # silently spliced into `entities` as a bare string.
    monkeypatch.setattr(embeddings, "get_provider", lambda: None)
    persona_store.save("lifestyle", {"hobbies": [
        {"name": "Climbing", "level": "casual"},
    ], "passions": [], "curiosities": [], "personality_traits": [],
        "values": [], "wellness": {}})
    good_id = persona_store.load("lifestyle")["hobbies"][0]["id"]
    bad_id = "project_deadbeef"
    ...
    bad_entry = out["entities"][1]
    assert set(bad_entry.keys()) == {"entity_id", "error"}
    assert bad_entry["entity_id"] == bad_id
